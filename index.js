...
bot.on('ready', () => {
    bot.user.setActivity(`${prefix}s (nickname)`, {type: 'PLAYING'});
});
    ...
    if (message.content.toUpperCase().startsWith(prefix)) {
        ...
        switch (String(args[1]).toUpperCase()) {
            default:
                ...
                try {
                    PLAYER = (await HypixelClient.getPlayer('name', args[1])).player
                }
                catch (err) {
                    console.log(err)
                    let errors = new Discord.RichEmbed()
                        .setColor(Math.floor(Math.random() * 16777214) + 1)
                        .addField('Error', 'Player not found')
                    message.channel.send(errors)
                    return
                }
                let embed1 = new Discord.RichEmbed()
                    .setColor(Math.floor(Math.random() * 16777214) + 1)
                    .setAuthor('page 1', 'http://visage.surgeplay.com/face/'+PLAYER['uuid'])
                ...
                let embed2 = new Discord.RichEmbed()
                    .setColor(Math.floor(Math.random() * 16777214) + 1)
                    .setAuthor('page 2', 'http://visage.surgeplay.com/face/'+PLAYER['uuid'])
                    .setTitle(PLAYER['displayname']).setURL('https://plancke.io/hypixel/player/stats/'+PLAYER['displayname'])
                    ...
                    .addBlankField()
                    .addField('Teams Kills', '`'+IntForm(PLAYER['stats']['SuperSmash']['deaths_kills'])+'`', true)
                    .addField('Teams Deaths', '`'+IntForm(PLAYER['stats']['SuperSmash']['deaths_2v2'])+'`', true)
                    .addField('Teams Wins', '`'+IntForm(PLAYER['stats']['SuperSmash']['wins_2v2'])+'`', true)
                    .addField('Teams Losses', '`'+IntForm(PLAYER['stats']['SuperSmash']['losses_2v2'])+'`', true)
                    .addField('Teams K/D | W/L', '`'+(PLAYER['stats']['SuperSmash']['kills_2v2'] / PLAYER['stats']['SuperSmash']['deaths_2v2']).toFixed(2)+'` | `'+(PLAYER['stats']['SuperSmash']['wins_2v2'] / PLAYER['stats']['SuperSmash']['losses_2v2']).toFixed(2)+'`', true)
                    .setThumbnail('https://hypixel.net/styles/hypixel-uix/hypixel/game-icons/BedWars-64.png')
                let embed3 = new Discord.RichEmbed()
                    .setColor(Math.floor(Math.random() * 16777214) + 1)
                    .setAuthor('page 3', 'http://visage.surgeplay.com/face/'+PLAYER['uuid'])
                    .setTitle(PLAYER['displayname']).setURL('https://plancke.io/hypixel/player/stats/'+PLAYER['displayname'])
                ...
                message.channel.send(embed1).then(async msg => {
                    ...
                    await msg.react('👉').then(async r => {
                        ...
                        const delwardsFilter = (reaction, user) => reaction.emoji.name === '🔴' && user.id === message.author.id;
                        const forwardsFilter = (reaction, user) => reaction.emoji.name === '👉' && user.id === message.author.id;
            
                        const backwards = msg.createReactionCollector(backwardsFilter, {time: 60000});
                        const delwards = msg.createReactionCollector(delwardsFilter, {time: 60000});
                        ...
                        delwards.on('collect', (reaction, user) => {
                            msg.delete()
                        })
...
bot.login('NjE3NjcwNjgxODg2MzI2Nzg0.XXZo3w.kqEL1FmjudA0WucyZbD3NzoDxcE');
